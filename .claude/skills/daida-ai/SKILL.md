...
# 次にスクリプトで所定のパスに保存
.venv/bin/python .claude/skills/daida-ai/scripts/generate_outline.py output/outline.md --stdin < output/outline_draft.md
```
...
# 次にスクリプトでバリデーション＆保存
.venv/bin/python .claude/skills/daida-ai/scripts/enrich_outline.py output/slide_spec.json --stdin < output/slide_spec_draft.json
```
...
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/create_slides.py output/slide_spec.json output/presentation.pptx
```

カスタムテンプレートを使う場合:
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/create_slides.py output/slide_spec.json output/presentation.pptx --template path/to/template.pptx
```
...
1. 現在のノートを読み出す:
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/write_talk_script.py output/presentation.pptx --read
```
...
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/write_talk_script.py output/presentation.pptx --notes-json output/notes.json --output output/presentation.pptx
```
...
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/synthesize_audio.py output/presentation.pptx output/audio/ --engine edge
```

VOICEVOX使用時（事前にVOICEVOX Engineの起動が必要）:
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/synthesize_audio.py output/presentation.pptx output/audio/ --engine voicevox
```
...
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/embed_audio.py output/presentation.pptx output/audio/ output/presentation_with_audio.pptx
```
...
ODP形式が必要な場合:
```bash
.venv/bin/python .claude/skills/daida-ai/scripts/convert_format.py output/presentation.pptx --outdir output/
```
